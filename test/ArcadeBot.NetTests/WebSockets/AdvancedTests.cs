...
public class AdvancedTests
{
    ...
    public async Task StreamFakeMessage_CustomMessage_ShouldBeFakelyStreamed()
    {
        ...
        client.StreamFakeMessage(SocketResponse.TextMessage(null));
        client.StreamFakeMessage(SocketResponse.TextMessage(myMessage));

        receivedEvent.WaitOne(TimeSpan.FromSeconds(30));

        Assert.Equal(myMessage, received);
        Assert.Throws<ArgumentNullException>(() => client.StreamFakeMessage(null));
    }
